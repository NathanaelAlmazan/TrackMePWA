...
import { RouterLink, Logo } from '../components';
...
export default function Nav({ openNav, onCloseNav }: { openNav: boolean, onCloseNav: () => void }) {
  ...
  const { firstName, lastName, position, role, avatar } = useAppSelector((state) => state.auth);
  ...
  const renderMenu = (
    <Stack component="nav" spacing={0.5} sx={{ px: 2 }}>
      {role && navConfig.filter(item => item.roles === null || item.roles?.includes(role)).map((item) => (
        <NavItemContainer key={item.title} item={item} />
      ))}
    </Stack>
  );
...
function NavItemContainer({ item }: { item: NavItem }) {
  const pathname = usePathname();
