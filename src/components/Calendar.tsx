...
import { Events, Frequency, Status } from "../__generated__/graphql";
...
export function filterEvents(events: Events[], date: Date) {
    return events.filter((event) => {
        ...
        if (event.frequency === Frequency.Monthly) 
            return (new Date(event.date).getDate() === date.getDate() || dateDue?.getDate() === date.getDate());
        return ((new Date(event.date).getMonth() === date.getMonth() || dateDue?.getMonth() === date.getMonth()) && 
            (new Date(event.date).getDate() === date.getDate() || dateDue?.getDate() === date.getDate()));
    });
}
                            ...
                            {week.map(day => (
                                ...
                                >
                                    <Stack spacing={1} sx={{ minHeight: 80 }}>
                                        {day.toDateString() === selectedDate.toDateString() ? (
                                            ...
                                            <Typography 
                                                ...
                                                sx={{ 
                                                    mb: 2,
                                                    cursor: 'pointer',
                                                    opacity: day.getMonth() === currentDate.getMonth() ? 1 : 0.5
                                                }}
                                            >
                                                {day.getDate()}
                                            </Typography>
                                        ...
                                        {events && filterEvents(events, day).map((event) => (
                                            <EventLabel 
                                                key={event.subject}
                                                date={day} 
                                                event={event} 
...
export function EventLabel({ 
    ...
    useEffect(() => {
        switch (event.type) {
            case 'DOCUMENT':
                getDocumentStatus({ variables: { referenceNum: event.id } })
                    .then(({ data }) => {
                        if (data && data.getDocumentById) {
                            setStatus(data.getDocumentById.status?.label || null);
                            setCompleted(data.getDocumentById.status?.category === Status.Finished)
            ...
            case 'HR_REPORT':
                if (event.id === event.subject) {
                    ...
                    getReportStatus({ variables: { id: parseInt(event.id)} })
                        .then(({ data }) => {
        ...
        <>
            <Typography 
                ...
                sx={{ 
                    ...
                    bgcolor: status ? completed ? 
                        ...
                        alpha(theme.palette.warning.light, 0.5) : alpha(theme.palette.primary.light, 0.5),
                    color: status ? completed ? 
                        theme.palette.success.dark : 
                        theme.palette.warning.dark : theme.palette.primary.dark,
                    borderRadius: '10px',
                    px: '8px',
                    py: '5px',
                    cursor: 'pointer'
            ...
            >
               <Stack spacing={2} sx={{ p: 2 }}>
                    ...
                    {redirect && (
                        <Box onClick={() => navigate(redirect)} sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end' }}>
                    ...
                    {(event.type === 'EVENT' && officeId === undefined) && (
                        <Stack direction='row' justifyContent='end'>
                            ...
                            <Tooptip title='Delete'>
                                <IconButton color='error' onClick={(e) => onDelete(e, parseInt(event.id))}>
