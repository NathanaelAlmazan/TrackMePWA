...
export function FormCollapse({
  ...
  return (
    <Collapse in={expanded} timeout="auto" unmountOnExit>
      <Box sx={{ py: 2 }}>
        <Form categories={categories} onSubmit={onCreate} onCancel={onCancel} />
      </Box>
    </Collapse>
  );
}
    ...
    <>
      {edit ? (
        ...
        >
          <ListItemText
            primary={value.label}
            secondary={value.category && capitalCase(value.category)}
          />
        </ListItem>
      )}
...
export function Form({
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selected)
      onSubmit({ id: selected.id, label, category }); // update selected item
